...
public class CardController {
	...
	@RequestMapping(value = "/card") // url과 method 매핑 시켜 준다.(http://localhost/list)가 오면 밑에 메서드가 시작된다는 뜻.
	public ModelAndView list(@RequestParam Map<String, Object> map) {
		List<Map<String, Object>> list = service.list(map);
		ModelAndView mv = new ModelAndView();
		mv.addObject("data", list); // (key, value)
		mv.setViewName("/card/card");
		return mv; // ModelAndView가 object 타입이니까 object를 리턴한다. (list.jsp로 포워드 하라는 뜻)
	}
